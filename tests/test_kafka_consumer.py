...
from kafka_consumer import load_config, create_kafka_consumer, create_kafka_producer, clean_data, process_messages
...
class TestKafkaConsumer(unittest.TestCase):
    ...
    def test_clean_data_unreliable_sensor(self):
        sensors = [20.5, 20.7, 20.6, 23.8]
        clean_value, status = clean_data(sensors)
        self.assertEqual(clean_value, 20.6)
        self.assertEqual(status, "UnreliableSensorReading")

    def test_clean_data_unreliable_row(self):
        sensors = [20.5, 20.7, 25.6, 25.8]
        clean_value, status = clean_data(sensors)
